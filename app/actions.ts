...
export async function createProduct(formData: FormDataType, email: string) {
    try {
        ...
        if (!email || !price || !categoryId || !name) {
            throw new Error("le nom, le prix, la catégorie et l'email de l'association sont requis pour la création de l'article.");
        }
...
export async function updateProduct(formData: FormDataType, email: string) {
    try {
        ...
        if (!email || !price || !id || !name) {
            throw new Error("l'id, le nom, le prix et l'email sont requis pour la mise à jour de l'article.");
        }
...
export async function deleteProduct(id: string, email: string) {
    try {
        if (!id) {
            throw new Error("l'id est requis pour la suppression de l'article.");
        }
    ...
    try {
        ...
        for (const item of orderItems) {
            ...
            if (product.quantity < item.quantity) {
                throw new Error(`Le produit"${product.name}" n'a pas de stock. Demandé: ${item.quantity}, Disponible: 
...
export async function getProductCategoryDistribution(email: string) {
     try {
        ...
        const categoriesWithProductsCount = await prisma.category.findMany({
            where: {
                associationId: association.id
            },
            include: {
          ...
          const data = categoriesWithProductsCount
          .map((category) => ( 
            {
            name: category.name,
